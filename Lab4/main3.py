...
def krit_studenta2(x, y, y_aver, n, m):
    S_kv = disp(y, y_aver, n, m)
...
def krit_studenta(x, y_average, n, m, dispersion):
...
def krit_fishera(y, y_average, y_new, n, m, d, dispersion):
...
def check(X, Y, B, n, m, norm=False):
    ...
    dispersion_arr = disp(Y, y_aver, n, m)
    qq = (1 + 0.95) / 2
    student_cr_table = t.ppf(df=f3, q=qq)
    ts = krit_studenta2(X[:, 1:], Y, y_aver, n, m)
    ...
    y_new = []
    for j in range(n):
        y_new.append(reg([X[j][i] for i in range(len(ts)) if ts[i] in res], final_k))
    ...
    if d >= n:
        ...
        return
    f4 = n - d
    Fp = krit_fishera(Y, y_aver, y_new, n, m, d, dispersion_arr)
...
def with_interaction_effect(n, m):
    ...
    y_aver = [round(sum(i) / len(i), 3) for i in Y]
    B_norm = find_coeff(X_norm, y_aver, norm=True)
...
def linear(n, m):
    ...
    y_average, B = reg_equation(x, y, n)
    dispersion_arr = disp(y, y_average, n, m)
    temp_cohren = f.ppf(q=(1 - q / f1), dfn=f2, dfd=(f1 - 1) * f2)
    cohren_cr_table = temp_cohren / (temp_cohren + f1 - 1)
    Gp = max(dispersion_arr) / sum(dispersion_arr)
    ...
    qq = (1 + 0.95) / 2
    student_cr_table = t.ppf(df=f3, q=qq)
    student_t = krit_studenta(x_norm[:,1:], y_average, n, m, dispersion_arr)
    print('Табличне значення критерій Стьюдента:\n', student_cr_table)
    ...
    y_new = []
    for j in range(n):
        y_new.append(reg([x[j][student_t.index(i)] for i in student_t if i in res_student_t], final_coefficients))
    print(f'Отримаємо значення рівняння регресії для {m} дослідів: ')
    print(y_new)
    d = len(res_student_t)
    f4 = n - d
    Fp = krit_fishera(y, y_average, y_new, n, m, d, dispersion_arr)
    Ft = f.ppf(dfn=f4, dfd=f3, q=1 - 0.05)
    print('Перевірка адекватності за критерієм Фішера:\n')
